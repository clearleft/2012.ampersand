	<header class="section clearfix" >
			<div class="ampersand-logo">
				<a href="/"><img src="img/ampersand-logo-<?php echo $lang; ?>.png" alt="Ampersand" /></a>
			</div>
			<h1 id="mainTitle" class="summary">A<span class="char2">m</span><span class="char3">p</span>e<span class="char5">r</span><span class="char6">s</span><span class="char7">a</span>nd</span>
			</h1>
			<div  class="event-info">
				<h2 id="description" class="description">The Web Typography<br /><strong>Confere<span class="char8">n</span>ce</strong></h2>
				<div id="whereWhen">
					<time datetime="2012-06-15" class="dtstart">15<sup>th</sup> June 2012,</time>
					<span  class="location">Brighton UK</span>
				</div>
			</div>
			<div class="tickets clearfix">
				<div id="ticketInfo">
					<span class="price"><span class="pound">&pound;</span><strong>125</strong><em>+</em>VAT</span>
					<?php
					
					DATE_DEFAULT_TIMEZONE_SET('GMT');  
					if ( time( ) > strtotime( "03/06/2012 09:59AM" ) ) {
    				
					?>
					<span class="buy"><a class="register-button" href="http://ampersand2012.eventbrite.co.uk/"><span></span>Sold Out!</a></span>
					<?php }else { ?>
					<span class="buy"><span class="brace brace1">{</span><strong>Tickets go on sale 6th March 2012</strong><span class="brace brace2">}</span></span>		<?php } ?>
				</div>
			</div>
			<nav class="main-nav clearfix">
				<ul>
					<li><a class="home" href="/" >Home</a></li>
					<li><a class="schedule" href="/schedule" >Schedule</a></li>
					<li><a class="location" href="/location" >Location</a></li>
					<li><a class="sponsors" href="/sponsors" >Sponsors</a></li>
				</ul>
				<span></span>
			</nav>
	</header>

